...
═══ C4 / РИСК3: me/ — ОПЦИОНАЛЬНОЕ обогащение, НЕ путь ЗНАНИЯ ═══
Чтение `me/*.md` здесь (`sync()` / `collect_candidates`) — это ASR-обогащение
именами собственными из прозы владельца, БОНУС-слой. Само ЗНАНИЕ бота (глоссарий
терминов + оргструктура/ростер ролей), которым он «умён», живёт в `*-context` и
читается через `lib/context_knowledge` (проекция A здесь — `sync_glossary_vocab`),
БЕЗ `me/`. Поэтому при отсутствии `me/` бот остаётся умным (C4): термины/роли
приходят из `*-context`, а это обогащение просто тихо пропускается (отсутствующий
файл/каталог → пустой список, не падаем, не зануляем — см. `collect_candidates`).
Инвариант «работает без папки владельца» закреплён тестом
`tests/test_knowledge_me_independence.py`. Сырьё (транскрипты/протоколы) остаётся
в `me/встречи` — это норма (РИСК1 снят), а не зависимость функции знания.
