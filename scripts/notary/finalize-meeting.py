...
def main() -> int:
    ...
    # Сам критик — ОДИН проход (НЕ плодит 3-й проход на каждый check). С best-of-2
    # (ниже) на finalize тяжёлых вызовов три: генерация A + генерация B + этот
    # критик-склейщик; на редких путях (команда/CLI) — два (генерация + критик).
    # РИСК1: таймаут/сбой второго прохода → черновик отдаётся как финал (деградация,
    # не потеря встречи).
