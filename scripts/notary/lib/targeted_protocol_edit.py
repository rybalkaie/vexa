...
# Потолок числа СЛОВ в X/Y. Терсный X/Y — термин/имя («frontend», «Михаил Сергеевич
# Еремеев», «ООО Ромашка»): несколько слов. Больше — это уже не термин, а поглощённый
# хвост предложения: «замени фронтенд на frontend и ещё там дата неправильная» жадно
# дало бы Y=«frontend и ещё там дата неправильная» (мусорная замена + молчком потеряна
# вторая правка). >потолка слов или клаузо-разделитель «, » в X/Y → отказ → регенерация
# разберёт реплику целиком. Ложный отказ дёшев, ложное применение опасно (R2).
_MAX_REPLACE_TERM_WORDS = 4
...
# Самостоятельный токен «на» (предлог) — для детекции МНОГОСВЯЗНОЙ неоднозначности
# глагольной директивы. «на» в русском — частый предлог, и X нежадно делится по
# ПЕРВОМУ « на »: «замени налоги на прибыль на НДС» распарсилось бы как X=«налоги»,
# Y=«прибыль на НДС» (мис-парс, X≠задуманному «налоги на прибыль»). Если в директиве
# больше одного самостоятельного «на» — какой из них разделитель X|Y детерминированно
# неизвестно → отказ (R2: ложное применение опаснее лишней регенерации). Word-boundary
# `(?<!\w)`/`(?!\w)` — Unicode-aware (кириллица=`\w`), поэтому «на» внутри «налоги»/
# «Татьяна»/«начало» НЕ считается. Только для глагольной формы (у стрелочной «X → Y»
# разделитель — стрелка, и «на» внутри X/Y там легитимен).
_NA_SEP_RE = re.compile(r"(?<!\w)на(?!\w)", re.IGNORECASE | re.UNICODE)
...
def _parse_one_directive(text: str) -> Optional[tuple]:
    ...
    if not t or len(t) > _MAX_REPLACE_DIRECTIVE_LEN:
        return None
    m = _DIRECTIVE_RE.match(t)
    if m is not None:
        # Глагольная форма «<глагол> X на Y»: >1 самостоятельного «на» (предлог) →
        # точка деления X|Y неоднозначна → отказ → следующий ярус (Ф2/регенерация
        # поймёт смысл целиком). См. _NA_SEP_RE. Стрелочную форму это не касается.
        if len(_NA_SEP_RE.findall(t)) > 1:
            return None
    else:
        m = _ARROW_RE.match(t)
        if m is None:
    ...
    if len(x) > _MAX_REPLACE_TERM_LEN or len(y) > _MAX_REPLACE_TERM_LEN:
        return None
    # Хвостовая проза: клаузо-разделитель «, » (запятая+пробел — продолжение фразы,
    # не десятичная «1,5») или слишком много слов → X/Y перестал быть термином →
    # отказ (см. _MAX_REPLACE_TERM_WORDS). Иначе Y поглотил бы хвост и применил мусор.
    if ", " in x or ", " in y:
        return None
    if len(x.split()) > _MAX_REPLACE_TERM_WORDS or len(y.split()) > _MAX_REPLACE_TERM_WORDS:
        return None
    return x, y
