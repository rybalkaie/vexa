...
Если HF_TOKEN не задан или модель не загружена — поднимает RuntimeError
с инструкцией. Fallback на embedding+clustering без HF (например, Resemblyzer)
на Ф3 НЕ реализован — добавим, если владелец откажется получать HF-токен.
