...
ЗАВ1 / Ф5 — ВЫПОЛНЕНО. Источник ростера — оргструктура из `*-context` нужной
компании (`get_roster` → `context_knowledge.roster_for_series`, поиск slug по всем
компаниям). `_STATIC_ROSTERS` ниже остался ПЕРЕХОДНЫМ fallback'ом (graceful
degradation, когда клон `*-context` ещё не забутстраплен — control/Ф8), НЕ
источником истины: боевой Anzhee-ростер живёт в
`anzhee-context/knowledge/notary/org-structure.yaml`. Менять источник надо ТОЛЬКО
в теле `get_roster`; сигнатуры/контракт (`map_from_roster_domain`,
`format_roster_hint`, тесты) от источника не зависят.
