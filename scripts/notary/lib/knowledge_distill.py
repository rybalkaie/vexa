...
# Заголовок Ф8-хвоста — вырезаем перед дистилляцией (это НЕ факт встречи).
_CARRYOVER_HEADER_RE = re.compile(r"^\s*#{1,6}\s*🔻", re.MULTILINE)
...
def strip_carryover(protocol_text: str) -> str:
    ...
    nxt = re.search(r"^\s*#{1,6}\s+(?!🔻)", text[m.end():], re.MULTILINE)
    if nxt:
        end = m.end() + nxt.start()
