    ...
    *,
    ...
    if remap and not content_edits:
        try:
            ...
            # warning (не info): исключение здесь = СЛОМАЛСЯ targeted-модуль (баг/дрейф),
            # а не штатный None-фолбэк. В headless-автономе тихая деградация (всё молча
            # уходит в регенерацию → «плавание» вернулось) иначе тонет в info. Текст
            # реплик не логируем — str(e) от re/dict-операций его не содержит (опасная тройка).
